...
(function ($,mc) {
	...
	function checkIfCleardownRequired() {
		...
		if (
			...
			var msg1 = 'Later version(s) of the process(es) being tested have been downloaded. ';
	...
	function processProvisioned(o) {
		...
		function success() {
			...
			function ppaDataLoad() {
				var ppaLoadingPage = $('#ppaLoadingPage');
			...
			function transitionToProcessesOrHomePage() {
				if (MCAPTURE_CFG.haveHomeProcess) {
					success2();
				} else {
			...
			if (MCAPTURE_CFG.appType == 'ppa' && MCAPTURE_CFG.havePpaData) {
				if (mc.ppa.dataLoadRequired()) {
					...
					transitionToProcessesOrHomePage();
				}
			} else {
				transitionToProcessesOrHomePage();
			}
				...
				(function($, mc) {
					...
					function isReady() {
						...
						function clear(e) {
							...
							mc.msgBox(function(text) {
								if (text === 'Yes') {
									window.locationUtils.setHref('../../clear.html?newGuid=no&amp;clearType=full');
								}
						...
						if (MCAPTURE_CFG.deviceFormat == 'tablet') {
							mc.ready(false, 1024, 768, process);
						} else {
							mc.ready(false, 320, 480, process);
						}
