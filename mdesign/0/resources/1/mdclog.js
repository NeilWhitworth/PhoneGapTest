...
var log4javascript = (function() {
	...
	EventSupport.prototype = {
		...
		setEventTypes: function(eventTypesParam) {
			if (eventTypesParam instanceof Array) {
				...
				this.eventListeners = {};
				for ( i = 0, len = this.eventTypes.length; i < len; i++) {
	...
	Level.TRACE = new Level(5000, "TRACE");
	Level.DEBUG = new Level(10000, "DEBUG");
	Level.INFO = new Level(20000, "INFO");
	Level.WARN = new Level(30000, "WARN");
	Level.ERROR = new Level(40000, "ERROR");
	Level.FATAL = new Level(50000, "FATAL");
/*
...
*/
	...
	function Logger(name) {
		...
		this.callAppenders = function(loggingEvent) {
			...
			for ( i = 0, len = effectiveAppenders.length; i < len; i++) {
		...
		this.getEffectiveLevel = function() {
			for ( logger = this; logger !== null; logger = logger.parent) {
	...
	var ROOT_LOGGER_DEFAULT_LEVEL = Level.TRACE;
	...
	function NullLayout(combineMessages) {
	...
	NullLayout.prototype.format = function(loggingEvent) {
		if (this.combineMessages) {
			return loggingEvent.getCombinedMessages();
		}
...
/* ---------------------------------------------------------------------- */
	...
	function formatObjectExpansion(obj, depth, indentation) {
		...
		function doFormat(obj, depth, indentation) {
			...
			function formatString(text) {
				...
				for ( j = 1, jLen = lines.length; j < jLen; j++) {
	...
	BrowserConsoleAppender.prototype.append = function(loggingEvent) {
		...
		function logMessage() {
		    //use the appropriate logger if its available
		    //otherwise use the next higeher level logger
		    //we've already established that we want to see the log lines through the level check
		    //so we dont want to loos them now.
		    //BEWARE
		    //I tried to use a simpler method, but it screwed up logging in iOS cordova
		    //returned the appropriate logger function if it existed and then called that
			...
			//if nothing else use any that we can
		    if (window.console.warn) {
			    window.console.warn(formattedMesage);
			    return;
			}
			...
			alert(formattedMesage);
	}
        if (window.console ) { 
			if (Level.TRACE.isGreaterOrEqual(loggingEvent.level)) {
				logMessage();
			} else if (Level.DEBUG.equals(loggingEvent.level)) {
				logMessage();
			} else if (Level.INFO.equals(loggingEvent.level)) {
				logMessage();
			} else if (Level.WARN.equals(loggingEvent.level)) {
				logMessage();
			} else if (loggingEvent.level.isGreaterOrEqual(Level.ERROR)) {
				logMessage();
			} else
				logMessage();
		} else {
	...
	function CordovaLogAppender() {}

	CordovaLogAppender.prototype = new log4javascript.Appender();
	CordovaLogAppender.prototype.layout = new NullLayout();
	CordovaLogAppender.prototype.threshold = Level.TRACE;

	CordovaLogAppender.prototype.append = function(loggingEvent) {
		var appender = this;
		...
		if( window.plugins && window.plugins.log ) {					    
			window.plugins.log.log(loggingEvent.logger.name, loggingEvent.level.level, getFormattedMessage());			
		}
	};	

	CordovaLogAppender.prototype.toString = function() {
		return "CordovaLogAppender";
	};

	log4javascript.CordovaLogAppender = CordovaLogAppender;	

	/* ---------------------------------------------------------------------- */
	// NotifyLogAppender

	function NotifyLogAppender() {}

	NotifyLogAppender.prototype = new log4javascript.Appender();
	NotifyLogAppender.prototype.layout = new NullLayout();
	NotifyLogAppender.prototype.threshold = Level.TRACE;

	NotifyLogAppender.prototype.append = function(loggingEvent) {
		try {
			var appender = this;
			var getFormattedMessage = function() {
				var layout = appender.getLayout();
			...
			if (window.mCapture && window.mCapture.notify && window.mCapture.notify.sendLogMessage) {
				window.mCapture.notify.sendLogMessage(loggingEvent.logger.name, loggingEvent.level.name, getFormattedMessage(), appender._configuration);
			}
		} catch (e) {
	...
	NotifyLogAppender.prototype.toString = function() {
		return "NotifyLogAppender";
	};

	NotifyLogAppender.prototype.configure = function (channel, subchannel, notification, timeToLive) {
		this._configuration = {
			_channel: channel,
	...
	log4javascript.NotifyLogAppender = NotifyLogAppender;	
	...
	WindowAppender.prototype = new Appender();

	WindowAppender.prototype.layout = new SimpleLayout();

	WindowAppender.prototype.append = function(loggingEvent) {
		var formattedMessage = this.getLayout().format(loggingEvent);
		if (this.getLayout().ignoresThrowable()) {
			formattedMessage += loggingEvent.getThrowableStrRep();
		}
		//alert(formattedMessage);
		div = document.getElementsByClassName('jmuiColumn')[0];
		if (div) {
			div.style.fontSize="x-small";
			div.style.visibility = "visible";
			div.setAttribute('style', 'display:block;');
			txt = document.createTextNode(formattedMessage );
			div.appendChild(txt); 
	...
	WindowAppender.prototype.toString = function() {
		return "WindowAppender";
	};

	log4javascript.WindowAppender = WindowAppender;

	/* ---------------------------------------------------------------------- */
	// HttpPostAppender

	function HttpPostAppender() {}

	HttpPostAppender.prototype = new Appender();

	HttpPostAppender.prototype.layout = new HttpPostDataLayout();

	HttpPostAppender.prototype.append = function(loggingEvent) {
		var formattedMessage = this.getLayout().format(loggingEvent);
		if (this.getLayout().ignoresThrowable()) {
			formattedMessage += loggingEvent.getThrowableStrRep();
		}
	  xmlhttp = new XMLHttpRequest();
		xmlhttp.open("POST","/Logger/" + formattedMessage  ,false);
		xmlhttp.send();
	};

	HttpPostAppender.prototype.toString = function() {
		return "HttpPostAppender";
	};

	log4javascript.HttpPostAppender = HttpPostAppender;
...
** END OF LOG4MDC.JS
*/
//Here we need to set up the log appenders that are chosen in the settings
//and the levels that are chosen
(function () {
  function removeAppender(logger,name) {
      var a, i;
      if (name && log4javascript.hasOwnProperty(name)) {
          try {
              a = logger.getEffectiveAppenders();
              if (a && a.length) {
                  for (i = 0; i < a.length; i += 1) {
                      try {
                          if (a[i] instanceof log4javascript[name]) {
                              logger.removeAppender(a[i]);
                          }
                      } catch (ex) {
              ...
              log.error('Unable to remove invalid appender "' + name + '" in removeAppender');
          }
      }
  }
	function cordovaReady() {
		// If we want a cordova appender, here's where we do it
		log.debug('CORDOVA READY EVENT');
		if (window.plugins && window.plugins.log) {					    
			//removeAppender(log,'WindowAppender');
			//removeAppender(applog,'WindowAppender');
		} else {
			log.error('No cordova log object available');
		}
	}
	log = log4javascript.getLogger('framework');	// Global variable (yuk) log
	if (log) {
		log.setLevel(log4javascript.Level.TRACE);
		log.addAppender(new log4javascript.BrowserConsoleAppender());
		//log.addAppender(new log4javascript.NotifyLogAppender());
		//log.addAppender(new log4javascript.WindowAppender());
		//log.addAppender(new log4javascript.HttpPostAppender());
		log.addAppender(new log4javascript.CordovaLogAppender());
	}
	applog = log4javascript.getLogger('application');	// Global variable (yuk) applog
	if (applog) {
	    applog.setLevel(log4javascript.Level.TRACE);
		applog.addAppender(new log4javascript.BrowserConsoleAppender());
		//applog.addAppender(new log4javascript.NotifyLogAppender());
		//applog.addAppender(new log4javascript.WindowAppender());
		//applog.addAppender(new log4javascript.HttpPostAppender());
		applog.addAppender(new log4javascript.CordovaLogAppender());
	}
	seclog = log4javascript.getLogger('secure');	// Global variable (yuk) applog
	if (seclog) {
	    seclog.setLevel(log4javascript.Level.TRACE);
		//seclog.addAppender(new log4javascript.BrowserConsoleAppender());
		//seclog.addAppender(new log4javascript.NotifyLogAppender());
		//seclog.addAppender(new log4javascript.WindowAppender());
		//seclog.addAppender(new log4javascript.HttpPostAppender());
		seclog.addAppender(new log4javascript.CordovaLogAppender());
	}
...
ï»¿(function($, mc) {
	...
	function ready() {
		...
		//debugger;
		mc = mCapture;
		mc.version.setversion('logging', mcversion);
	}
