...
/* EDD: This script has been modified to handle rotations when the signature pad is not visible */
		...
		this.eventTokens[apinamespace + '.parentresized'] = globalEvents.subscribe(
			...
			, (function(eventTokens, $parent, originalParentWidth, sizeRatio){
				...
				return function(){
					...
					var w = $parent.width();
					if (w < 1) {
						// EDD: Signature pad has no width, so do not attempt to resize it.
						return;
					}
...
function jSignatureClass(parent, options, instanceExtensions) {
	...
	, settings = {
		'width' : 'ratio'
		,'height' : 'ratio'
		,'sizeRatio': 2.5 // only used when height = 'ratio'
		,'color' : '#000'
	...
	var movementHandlers = (function(jSignatureInstance) {
		...
		this.drawStartHandler = function(e) {
			...
			/* Move away from an input that has focus to prevent the Keyboard being launched */
...
var GlobalJSignatureObjectInitializer = function(window){
	...
	;(function(globalEvents, apinamespace, $, window){
		...
		// We also want to perform a resize when we transition to a new page
		// so hook into the jmui page navigation event as well
		var jmfw = window.jmfw;
		jmfw('div.jmuiPage').bind('jmuiPageAnimationEnd', function(e) {
			if (e.direction === 'in') {
				if (resizetimer) {
					clearTimeout(resizetimer);
				}
	...
	function _clearDrawingArea( data ) {
		if(this.find('canvas.'+apinamespace).length > 0){
