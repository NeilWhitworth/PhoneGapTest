		...
		function getData(processId, summaryNames, conditions, distinct, sort, callback) {
			...
			function match(values) {
				...
				if (conditions && conditions.length) {
					for (i = 0; b && i < conditions.length; i += 1) {
						switch (conditions[i].condition) {
						case 'begins':
							if (!values[conditions[i].summaryName] || values[conditions[i].summaryName].indexOf(conditions[i].value) !== 0) {
								b = false;
							}
							break;
						case 'ends':
							if (!values[conditions[i].summaryName] || values[conditions[i].summaryName].lastIndexOf(conditions[i].value) !== values[conditions[i].summaryName].length - conditions[i].value.length) {
								b = false;
							}
							break;
						case 'contains':
							if (!values[conditions[i].summaryName] || values[conditions[i].summaryName].indexOf(conditions[i].value) === -1) {
								b = false;
							}
							break;
						default:
							if (values[conditions[i].summaryName] !== conditions[i].value) {
								b = false;
							}
							break;
						}
			...
			if (needsRefresh(processId, conditions, distinct, sort)) {
				...
				process = mc.db.getProcess(processName);
				if (sort && sort.column) {
					sortColumn = process.summaryMap.indexOf(sort.column);
				}
				tx = getTx(['subject']);
				tx.onerror = function (e) {
					callback([]);
				...
				if (cursor) {
					if (!cursor.value._meta.backupGuid && cursor.value._summary) {
						...
						if (options.filters && options.filters.length) {
							$.each( options.filters, function( index, filter ) {							
								col = "col" + proc.summaryMap.indexOf(filter.summaryName);
								v = cursor.value._summary[col];
								if (separator !== undefined && separator === 1) {
									if (!v || v.indexOf(separator + filter.summaryValue) !== 0) {
										include = false;
									}
								} else {
						...
						if (include) {
							...
							//v = cursor.value._summary[colName];						<<<<----- What is this doing here?
							//if (!options.distinct || a.indexOf(v) === -1) {
							//	a.push(v);
							//}
						}
					}
